import pytest
from tele_pulse import RRPauseDetectorAgent, EctopyFrequencyAgent, QTcDriftTrackerAgent, TelemetryCoordinator, main


def test_sub_agents():
    a1 = RRPauseDetectorAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = EctopyFrequencyAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = QTcDriftTrackerAgent()
...
def test_domain_registry():
    from tele_pulse import DomainKnowledgeRegistry
